...
- **Spec §10.9 observability enforcement** (phase 14) — the `add-observability` skill bumps to `0.3.0` / `implements_spec: 0.3.0` and gains the three primitives §10.9 mandates: delta scan, baseline file, CI workflow.
...
- **`scan-apply/APPLY.md` Phase 6b** (§10.9.2) — regenerates `baseline.json` automatically after a successful apply. Skipped when zero findings were applied (the source code is unchanged; baseline is already accurate).
- **Reference CI workflow `add-observability/ci/observability.yml`** (§10.9.3) — GitHub Actions workflow shipping SHA-pinned actions (`actions/checkout@de0fac2…` v6.0.2, `marocchino/sticky-pull-request-comment@0ea0beb…` v3.0.4), env-var indirection for every `${{ }}` interpolation inside `run:` blocks, top-level `permissions: contents: read`, concurrency block, `pull_request` trigger (NEVER `pull_request_target`). Compares delta gaps against base-branch baseline; fails PR on increase; surfaces findings as a sticky PR comment. Logs `::warning::enforcement disabled` (NOT silent) when baseline is missing or empty on the base branch — per §10.9.3 explicit-not-silent opt-out requirement.
- **`add-observability/ci/README.md`** — adoption guide documenting the v1.10.0 limitation (Claude Code in CI not fully supported on hosted runners; three workarounds), threat model (pull_request vs pull_request_target, prompt-injection mitigations, opt-out for fork PRs), opt-out path (delete or empty baseline.json), baseline merge-conflict guidance (regenerate, don't hand-merge), Dependabot example for keeping SHA pins fresh, customisation pattern (sibling-workflow-file).
...
- **Migration `0011-observability-enforcement.md`** — promotes 1.9.3 → 1.10.0 by (1) copying the reference CI workflow into `.github/workflows/observability.yml` (always overwrites; pre-existing different content is backed up to `.bak.<timestamp>`), (2) authoring initial `.observability/baseline.json` via `claude /add-observability scan --update-baseline`, (3) bumping `observability.spec_version` 0.2.1 → 0.3.0 and adding the new `enforcement:` sub-block to CLAUDE.md (§10.8), (4) adding the per-PR enforcement command reference line, (5) bumping `.claude/skills/agentic-apps-workflow/SKILL.md` version. Hard pre-flight aborts on three preconditions (no observability metadata → "run init first"; no policy.md → "run init first"; no `claude` CLI → "install separately") rather than silent skips.
- **`migrations/test-fixtures/0011/` with 7 sandboxed scenarios** — 01-fresh-apply (before-state idempotency), 02-idempotent-reapply (after-state idempotency + jq schema-strictness check on baseline.json), 03-no-observability-metadata (pre-flight 1 abort), 04-no-policy-md (pre-flight 2 abort), 05-baseline-already-present (Step 2 idempotency catches pre-existing v0.3.0 baseline), 06-no-claude-cli (`requires.tool.claude.verify` abort), 07-existing-workflow-yml (backup + overwrite). `test_migration_0011()` stanza added to `run-tests.sh`. **7/7 pass.** Preflight verify-path audit reports 0011's `requires.skill.add-observability.verify` as expected-fail on dev machines that develop from source rather than the canonical install path (Phase 13 disclaimer — informational only).
- **CONTRACT-VERIFICATION.md** gains a 15-row v0.3.0 §10.9 coverage matrix mapping each MUST to its load-bearing artefact and verification evidence.
...
- **CI gate fail rule**: spec §10.9.3 says "fails if the count increases". The workflow's pragmatic encoding is `if delta_high > 0: exit 1` — equivalent because delta represents the net new high-confidence gaps a PR introduces, so the post-PR project total would be `baseline_high + delta_high`. Documented inline in the workflow.
- **Known limitation**: the reference workflow at `add-observability/ci/observability.yml` invokes `claude` in CI. Claude Code's CI installation story is not fully supported on hosted GitHub runners as of 2026-05. Three workarounds (manual pre-PR scan, self-hosted runner, wait for v1.11.0 Node scanner port) are documented in `ci/README.md`.
- **Out of scope, deferred**: pre-commit hook template (§10.9.4 MAY), standalone Node scanner port, GitLab / CircleCI workflows, retroactive enforcement on fx-signal-agent, dashboard `baseline.json` rendering. Each is tracked as a v1.11.0 follow-up.
