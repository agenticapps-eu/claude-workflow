...
- **Hermetic test baseline.** The 0017 fixtures + `regen-hashes.sh` sourced the OLD wrapper from `git show main:`, which PR #45's merge turned into the NEW registry wrapper — silently making the whole suite a no-op. The OLD v0.4.x wrappers are now vendored as engine runtime data (`templates/.claude/scripts/migrate-0017-old-wrappers/`, shared by engine + tests). Suite 9/9; full migration suite 166.
