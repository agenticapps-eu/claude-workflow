 ...
 *
 * Phase 21 (P1, supabase-edge / Deno). A role-based registry lets the wrapper
 * route each observability ROLE (errors / logs / analytics) to a named
 ...
 *
 * RUNTIME NOTE (Deno / Supabase Edge): there is no Cloudflare
 * `ExecutionContext`. The registry keeps the `init(env, ctx?)` shape for
 * portability, but ctx is always undefined here — the Axiom adapter resolves
 * its fire-and-forget binding from the `EdgeRuntime.waitUntil` global instead
 * (guarded). Env is read from the wrapper, which assembles it from
 * `Deno.env.get(...)` (production) or an injected override (tests).
 */

import type { Envelope, InitEnv } from "../index.ts";
import { createSentryAdapter } from "./sentry.ts";
import { createAxiomAdapter } from "./axiom.ts";

// Cloudflare-compatible ExecutionContext shape, kept for cross-stack registry
// portability. Unused at runtime on Deno (ctx is always undefined here).
