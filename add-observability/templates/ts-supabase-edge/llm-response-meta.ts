...
// Sentry AI Monitoring doesn't surface (Supabase Edge / Deno variant).
//
...
// destination adapter.
//
// Deno note: import path uses explicit `.ts` extension (supabase-edge
// source dir has index.ts directly — no harness rename — per CONTEXT D-04a).

import type { Envelope } from "./index.ts";

/**
 * Function shape for emitting structured log envelopes.
 * Matches the signature of `logEvent` exported by ./index — declared locally
 * (not imported) for testability + §10.6 portability.
 */
export type LogEventFn = (envelope: Envelope) => void;

/**
 * OpenAI-SDK-compatible usage shape, as returned by OpenAI / OpenRouter
 * when the OpenAI SDK is used. All fields optional → defensive defaults to 0.
 *
 * Verified against:
