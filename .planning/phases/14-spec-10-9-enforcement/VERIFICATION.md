...
| Must-have | Artefact | Evidence command + result |
|---|---|---|
| Reference workflow shipped | `add-observability/ci/observability.yml` | `python3 -c 'import yaml; yaml.safe_load(open("..."))'` → parses |
| (1) Delta scan on every PR | observability.yml `Delta scan (PR)` step with `if: github.event_name == 'pull_request'` | inline |
| (2) Compare delta count vs base-branch baseline | `Read base baseline` step reads `git show ${BASE_SHA}:.observability/baseline.json`; `Compare delta vs baseline` step diffs counts | inline |
| (3) Fail PR if count would increase | `Compare delta vs baseline` step: `if [ "$D" -gt 0 ]; then exit 1` (equivalent to: post-PR project total > base baseline) | inline |
| (4) Surface findings as PR comment | `Comment on PR (on failure)` step using SHA-pinned `marocchino/sticky-pull-request-comment@0ea0beb...` | inline |
| No silent opt-out | "Read base baseline" emits `::warning::enforcement disabled — no baseline on base branch` or `…baseline.json on base branch is malformed`; logged to GHA run summary + Checks tab | `grep -q 'enforcement disabled' add-observability/ci/observability.yml` → 2 matches |
| Pinned action SHAs (no floating tags) | `actions/checkout@de0fac2...` (v6.0.2), `marocchino/sticky-pull-request-comment@0ea0beb...` (v3.0.4) | `grep -oE 'uses: [^@]+@[a-f0-9]{40}' observability.yml` → 2 SHA pins |
| `pull_request` trigger (NEVER `pull_request_target`) | `on.pull_request.branches: [main]`; cautionary comment + README threat model | `grep -v '^\s*#' observability.yml \| grep -E '\bpull_request_target\b'` → empty (no active use) |
...
| Must-have | Artefact | Evidence command + result |
|---|---|---|
| Migration applies cleanly | `migrations/0011-observability-enforcement.md` + 7 fixtures + run-tests stanza | `bash migrations/run-tests.sh 0011` → **7/7 PASS** |
...
| Step 1 always overwrites; backs up pre-existing different content | Step 1 Apply block with `cmp -s` + `cp ...bak.<timestamp>` | fixture 07 confirms backup + overwrite |
| Step 2 invokes scan with `--update-baseline` | Step 2 Apply block | fixture 01 + stubbed `claude` writes canned baseline.json on the right command |
| Step 5 SKILL.md version bump | Step 5 `sed` with idempotency check | fixture 02 `grep -q '^version: 1.10.0$'` → ok |
...
- ☑ Spec §10.9 fully implemented (delta scan, baseline file, CI workflow).
- ☑ Migration 0011 ships with 7-fixture test suite (7/7 PASS).
...
- ☑ Multi-AI review (codex + gemini + Claude) completed; 21-item revision pass applied; verdict APPROVE.
...
- ☑ CHANGELOG entry for v1.10.0 with full attribution and known limitations.
- ☑ CONTRACT-VERIFICATION.md gains 15-row v0.3.0 coverage matrix.
