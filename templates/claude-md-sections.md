# DEPRECATED — vendored as `.claude/claude-md/workflow.md` since v1.8.0

> **Do not paste this file into a project's CLAUDE.md.** It is kept here as
> a build artifact for migration `0000-baseline.md` (legacy reference) and
> for migration `0009`'s detection logic, which uses headings in this file
> to identify pre-existing inlined copies in older repos.
>
> The current canonical workflow content lives at
> `templates/.claude/claude-md/workflow.md` and is vendored into each
> project as `<repo>/.claude/claude-md/workflow.md`. CLAUDE.md links to
> that path instead of inlining the content. See ADR 0021.
>
> Earlier projects (set up before v1.8.0) inlined this file's content into
> their CLAUDE.md verbatim, including the prior H1 line "CLAUDE.md
> Sections — paste into your project's CLAUDE.md", which is the smoking
> gun migration 0009 grep-detects to offer extraction.
