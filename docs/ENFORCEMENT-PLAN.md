...
| **7 OpenSpec Change Gate** | `PreToolUse` | `Edit\|Write\|MultiEdit\|NotebookEdit` | Spec §18, gate **2.0.0**. Under an active OpenSpec change, blocks code edits unless `openspec validate --all` is green — that is the only blocking clause. `changes/<slug>/REVIEWS.md` coverage (absent, thin, stale, unverifiable, REQUEST-CHANGES) is **reported on every invocation and never blocks**. No active change → allow; `openspec/**` writes → exempt; malformed stdin → fail open. The project hook is a shim onto `~/.agenticapps/bin/openspec-change-gate.sh`; the same script runs as a git `pre-commit` hook and in CI, and neither of those goes red on the review count either. Retarget of the 0.x plan-review gate (ADR-0018 → ADR-0044) | `GSD_SKIP_REVIEWS=1` (logged; suppresses review reporting only). `OPENSPEC_GATE_STRICT=1` opts into the stricter "no code without a change" posture |
