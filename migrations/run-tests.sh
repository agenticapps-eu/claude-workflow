...
# Migration 0010's normalize-claude-md tests MOVED TO CORE.
#
# shim-project-hooks turned templates/.claude/hooks/normalize-claude-md.sh into
# a shim, so these golden tests began running against a script that no longer
# contains the behaviour they describe - cparx-shape went from 147 lines to 339
# because nothing normalized. Deleting them would have dropped real coverage, so
# the fixtures and the assertions moved with the implementation:
#
#   agenticapps-workflow-core/tools/normalize-claude-md.test.sh
#   agenticapps-workflow-core/tools/fixtures/normalize-claude-md/
#
# Seven-block normalization, the missing-source preserve, the 0009-vendored
# interaction, the line-count ceiling, CRLF input, atomic write and idempotency
# are all still asserted - 9/9 there, against the real implementation.
...
# The phase-sentinel tests are REMOVED, with the hook.
#
# shim-project-hooks deleted phase-sentinel.sh fleet-wide. It gated on
# .planning/current-phase/checklist.md, which existed in no repo in either
# family - permanently inert. There is nothing left to test, and unlike
# normalize-claude-md the behaviour did not move: see the DELETION-RECORD in
# agenticapps-workflow-core/openspec/changes/shim-project-hooks/.
