...
Brings projects from workflow v1.5.1 to v1.6.0 by installing the LLM wiki compiler plugin and configuring per-family wikis. Implements Andrej Karpathy's LLM Knowledge Base pattern. See ADR 0019 (to be written alongside this migration) for design rationale.
...
test "$INSTALLED" = "1.5.1" || { echo "ERROR: installed version is $INSTALLED, this migration requires 1.5.1"; exit 1; }
...
```bash
sed -i.bak 's/^version: 1\.5\.1$/version: 1.6.0/' .claude/skills/agentic-apps-workflow/SKILL.md
```
...
# Version bumped
grep -q '^version: 1.6.0$' .claude/skills/agentic-apps-workflow/SKILL.md || exit 1
...
sed -i.bak 's/^version: 1\.6\.0$/version: 1.5.1/' .claude/skills/agentic-apps-workflow/SKILL.md
```
