...
# raised to 0.8.0, config repointed + dangling hook ref dropped, dead hook
...
grep -q '^## Spec deltas (spec 0.8.0)' "$TARGET" \
...
d_line=$(grep -n '^## Spec deltas (spec 0.8.0)' "$TARGET" | cut -d: -f1)
...
# The §08 delta must be recorded as OPEN, not as resolved upstream. Core 0.8.0
# was a §04 clarification only; spec/08-migration-format.md is untouched since
# 0.1.0. A claim that core "amends §08" would be false — this assertion is what
# keeps the one section whose purpose is honest disclosure from lying.
grep -q 'not accepted' .actual-section \
  || { echo "STEP 2 failed: §08 delta must state the amendment is NOT accepted"; exit 1; }
grep -qi 'amends §08\|amended upstream' .actual-section \
  && { echo "STEP 2 failed: §08 delta falsely claims an upstream amendment"; exit 1; }
